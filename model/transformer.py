...
class QuantSoupTransformer(TransformerBase):
    ...
    def __init__(self, config, vq_config):
        ...
        self.extra_embeds = nn.Embedding(3,
                                         config.n_embd,
                                         padding_idx=self.padding_idx)
        self.transformer = nn.ModuleDict(
            dict(
                wpe=nn.Embedding(config.block_size, config.n_embd),
                wfie=nn.Embedding(self.finemb_size,
                                  config.n_embd,
                                  padding_idx=self.padding_idx),
                wfoe=nn.Embedding(self.foutemb_size,
                                  config.n_embd,
                                  padding_idx=self.padding_idx),
    ...
    def forward(self,
        ...
        embed = torch.zeros((b * t, self.config.n_embd),
                            dtype=torch.float32,
                            device=device)
        idx_in_extra = torch.isin(idx,
                                  torch.LongTensor([0, 1,
                                                    2]).to(device)).reshape(-1)
        idx_flat = idx.reshape(-1)
        embed[idx_in_extra, :] = self.extra_embeds(idx_flat[idx_in_extra])
        embed[~idx_in_extra, :] = self.input_layer(
            tokenizer.embed(idx_flat[~idx_in_extra] - 3))
        tok_emb = embed.reshape(b, t,
                                -1)  # token embeddings of shape (b, t, n_embd)
        finemb = self.transformer.wfie(
            fin)  # face inner embeddings of shape (t, n_embd)
        foutemb = self.transformer.wfoe(
            fout)  # face outer embeddings of shape (t, n_embd)
        ...
        if kv_cache is not None and kv_cache[0].numel():
            ...
            pos = torch.tensor([i for i in range(t)],
                               dtype=torch.long,
                               device=device)
    ...
    def generate(self,
        ...
        for iteration in range(max_new_tokens):
            ...
            if not use_kv_cache or (iteration == 0 and idx.shape[-1] > 1):
                ...
                fin_cond = current_fin if current_fin.size(
                    1) <= self.config.block_size else current_fin[:,
                                                                  -self.config.
                                                                  block_size:]
                fout_cond = current_fout if current_fout.size(
                    1
                ) <= self.config.block_size else current_fout[:, -self.config.
                                                              block_size:]
                fout_cond = torch.from_numpy(
                    get_shifted_sequence(fout_cond[0].cpu().numpy())).to(
                        idx_cond.device).unsqueeze(0)
            else:
                idx_cond = idx[:, -1:]
                fin_cond = current_fin[:, -1:]
                fout_cond = current_fout[:,
                                         -1:]  # note: don't need shifting since we assume block_size is huge enough to not need shifting
            ...
            if last_fin_cond == self.finemb_size - 1 or (iteration == 0 and
                                                         idx.shape[-1] == 2):
                current_fin = torch.cat(
                    (current_fin, (3 * one_t[0]).unsqueeze(0).unsqueeze(0)),
                    dim=1)
                current_fout = torch.cat(
                    (current_fout,
                     (current_fout[0, -1] + 1).unsqueeze(0).unsqueeze(0)),
                    dim=1)
            else:
                current_fin = torch.cat(
                    (current_fin,
                     (current_fin[0, -1] + 1).unsqueeze(0).unsqueeze(0)),
                    dim=1)
                current_fout = torch.cat(
                    (current_fout,
                     (current_fout[0, -1]).unsqueeze(0).unsqueeze(0)),
                    dim=1)
            if idx_next == 1:
                return idx
        return None
    ...
    @torch.no_grad()
    def generate_with_beamsearch(self,
                                 idx,
                                 fin,
                                 fout,
                                 tokenizer,
        ...
        idx = idx.repeat((beam_width, 1, 1)).transpose(0,
                                                       1).flatten(end_dim=-2)
        current_fin = current_fin.repeat(
            (beam_width, 1, 1)).transpose(0, 1).flatten(end_dim=-2)
        current_fout = current_fout.repeat(
            (beam_width, 1, 1)).transpose(0, 1).flatten(end_dim=-2)
        ...
        probabilities, top_k_indices = logits[0, 0, :].squeeze().log_softmax(
        ...
        if last_fin_cond == self.finemb_size - 1 or (idx.shape[-1] == 2):
            current_fin = torch.cat(
                (current_fin, (3 * one_t[0]).unsqueeze(0).unsqueeze(0).expand(
                    (beam_width, -1))),
                dim=1)
            current_fout = torch.cat(
                (current_fout,
                 (current_fout[0, -1] + 1).unsqueeze(0).unsqueeze(0).expand(
                     (beam_width, -1))),
                dim=1)
        else:
            current_fin = torch.cat(
                (current_fin,
                 (current_fin[0, -1] + 1).unsqueeze(0).unsqueeze(0).expand(
                     (beam_width, -1))),
                dim=1)
            current_fout = torch.cat(
                (current_fout,
                 current_fout[0, -1].unsqueeze(0).unsqueeze(0).expand(
                     (beam_width, -1))),
                dim=1)
        ...
        for iteration in tqdm(range(max_new_tokens - 1), desc='beam_search'):
            ...
            probabilities, top_k_indices = probabilities.topk(k=beam_width,
                                                              axis=-1)
            next_indices = torch.remainder(
                top_k_indices, vocabulary_size).flatten().unsqueeze(-1)
            best_candidates = (top_k_indices / vocabulary_size).long()
            best_candidates += torch.arange(
                idx.shape[0] // beam_width,
                device=idx.device).unsqueeze(-1) * beam_width
            idx = idx[best_candidates].flatten(end_dim=-2)
            for block_idx in range(len(kv_cache)):
                kv_cache[block_idx] = kv_cache[block_idx][:,
                                                          best_candidates.
                                                          flatten(), :, :, :]
            ...
            if last_fin_cond == self.finemb_size - 1 or (idx.shape[-1] == 2):
                current_fin = torch.cat(
                    (current_fin,
                     (3 * one_t[0]).unsqueeze(0).unsqueeze(0).expand(
                         (beam_width, -1))),
                    dim=1)
                current_fout = torch.cat(
                    (current_fout, (current_fout[0, -1] +
                                    1).unsqueeze(0).unsqueeze(0).expand(
                                        (beam_width, -1))),
                    dim=1)
            else:
                current_fin = torch.cat(
