...
class TriangleTokenizationGraphConv(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        if config.only_chairs:
            self.train_dataset = TriangleNodesWithFaces(
                config, 'train', config.scale_augment, config.shift_augment,
                '03001627')
            self.interesting_categories = [('03001627', "")]
            self.val_datasets = [
                TriangleNodesWithFaces(config, 'val', config.scale_augment_val,
                                       config.shift_augment_val, '03001627')
            ]
        else:
            self.train_dataset = TriangleNodesWithFaces(
                config, 'train', config.scale_augment, config.shift_augment,
                None)
            self.interesting_categories = [('02828884', '_bench'),
                                           ('02871439', '_bookshelf'),
                                           ('03001627', ""),
                                           ('03211117', '_display'),
                                           ('04379243', '_table')]
            ...
            for cat, name in self.interesting_categories:
                self.val_datasets.append(
                    TriangleNodesWithFaces(config, 'val',
                                           config.scale_augment_val,
                                           config.shift_augment_val, cat))
        self.encoder = GraphEncoder(no_max_pool=config.g_no_max_pool,
                                    aggr=config.g_aggr,
                                    graph_conv=config.graph_conv,
                                    use_point_features=config.use_point_feats,
                                    output_dim=576)
        self.decoder = resnet34_decoder(512, config.num_tokens - 2,
                                        config.ce_output)
        ...
        self.vq = ResidualVQ(
            ...
            commitment_weight=self.config.
            embed_loss_weight,  # the weight on the commitment loss
            stochastic_sample_codes=True,
            sample_codebook_temp=config.
            stochasticity,  # temperature for stochastically sampling codes, 0 would be equivalent to non-stochastic
            shared_codebook=self.config.embed_share,
            decay=self.config.code_decay,
        )
        self.register_buffer(
            'smoothing_weight',
            torch.tensor([2, 10, 200, 10, 2],
                         dtype=torch.float32).unsqueeze(0).unsqueeze(0))
        # print('compiling model...')
        # self.model = torch.compile(model)  # requires PyTorch 2.0
        self.output_dir_image_val = Path(
            f'runs/{self.config.experiment}/image_val')
        self.output_dir_image_val.mkdir(exist_ok=True, parents=True)
        self.output_dir_mesh_val = Path(
            f'runs/{self.config.experiment}/mesh_val')
        self.output_dir_mesh_val.mkdir(exist_ok=True, parents=True)
        self.output_dir_image_train = Path(
            f'runs/{self.config.experiment}/image_train')
        self.output_dir_image_train.mkdir(exist_ok=True, parents=True)
        self.output_dir_mesh_train = Path(
            f'runs/{self.config.experiment}/mesh_train')
    ...
    def configure_optimizers(self):
        parameters = list(self.encoder.parameters()) + list(
            self.decoder.parameters()) + list(
                self.pre_quant.parameters()) + list(
                    self.post_quant.parameters()) + list(self.vq.parameters())
    ...
    def create_conv_batch(self, encoded_features, batch, batch_size):
        return create_conv_batch(encoded_features, batch, batch_size,
                                 self.device)
    ...
    def training_step(self, data, batch_idx):
        ...
        encoded_x = self.distribute_features_fn(encoded_x, data.faces,
                                                data.num_vertices.sum(),
                                                self.device)
        ...
        encoded_x_conv, conv_mask = self.create_conv_batch(
            encoded_x, data.batch, self.config.batch_size)
        decoded_x_conv = self.decoder(encoded_x_conv)
        if self.config.ce_output:
            decoded_x = decoded_x_conv.reshape(
                -1, decoded_x_conv.shape[-2],
                decoded_x_conv.shape[-1])[conv_mask, :, :]
            decoded_tri = softargmax(decoded_x) / (self.config.num_tokens -
                                                   3) - 0.5
            _, decoded_normals, decoded_areas, decoded_angles = create_feature_stack_from_triangles(
                decoded_tri.reshape(-1, 3, 3))
            if self.config.use_smoothed_loss:
                otarget = torch.nn.functional.one_hot(
                    data.y.reshape(-1),
                    num_classes=self.config.num_tokens - 2).float()
                otarget = otarget.unsqueeze(1)
                starget = torch.nn.functional.conv1d(otarget,
                ...
                if self.config.use_multimodal_loss:
                    starget_a = starget.reshape(
                        -1, decoded_x.shape[-2] * decoded_x_conv.shape[-1])
                    starget_a = torch.nn.functional.normalize(
                        starget_a, p=1.0, dim=-1, eps=1e-12).squeeze(1)
                    starget_b = torch.nn.functional.normalize(
                        starget, p=1.0, dim=-1, eps=1e-12).squeeze(1)
                    loss = torch.nn.functional.cross_entropy(
                        decoded_x.reshape(
                            -1,
                            decoded_x.shape[-2] * decoded_x_conv.shape[-1]),
                        starget_a).mean()
                    loss = loss * 0.1 + torch.nn.functional.cross_entropy(
                        decoded_x.reshape(-1, decoded_x.shape[-1]),
                        starget_b).mean()
                else:
                    starget = torch.nn.functional.normalize(
                        starget, p=1.0, dim=-1, eps=1e-12).squeeze(1)
                    loss = torch.nn.functional.cross_entropy(
                        decoded_x.reshape(-1, decoded_x.shape[-1]),
                        starget).mean()
            ...
            loss_tri = torch.nn.functional.mse_loss(decoded_tri,
                                                    y_coords,
                                                    reduction='mean')
            loss_normals = torch.nn.functional.mse_loss(decoded_normals,
                                                        data.x[:, 9:12],
                                                        reduction='mean')
            loss_areas = torch.nn.functional.mse_loss(decoded_areas,
                                                      data.x[:, 12:13],
                                                      reduction='mean')
            loss_angles = torch.nn.functional.mse_loss(decoded_angles,
                                                       data.x[:, 13:16],
                                                       reduction='mean')
        else:
            decoded_x = decoded_x_conv.reshape(
                -1, decoded_x_conv.shape[-1])[conv_mask, :]
            _, decoded_normals, decoded_areas, decoded_angles = create_feature_stack_from_triangles(
                decoded_x.reshape(-1, 3, 3))
            loss = torch.nn.functional.mse_loss(decoded_x,
                                                data.y,
                                                reduction='mean')
            loss_tri = torch.nn.functional.mse_loss(decoded_x,
                                                    data.y,
                                                    reduction='mean')
            loss_normals = torch.nn.functional.mse_loss(decoded_normals,
                                                        data.x[:, 9:12],
        ...
        if (batch_idx + 1) % self.config.gradient_accumulation_steps == 0:
            step(optimizer,
                 [self.encoder, self.decoder, self.pre_quant, self.post_quant])
    ...
    def validation_step(self, data, batch_idx, dataloader_idx):
        ...
        if self.config.ce_output:
            decoded_x = decoded_x_conv.reshape(
                -1, decoded_x_conv.shape[-2],
                decoded_x_conv.shape[-1])[conv_mask, :, :]
            decoded_c = softargmax(decoded_x) / (self.config.num_tokens -
                                                 3) - 0.5
            loss = torch.nn.functional.cross_entropy(decoded_x.reshape(
                -1, decoded_x.shape[-1]),
                                                     data.y.reshape(-1),
                                                     reduction='mean')
            y_coords = data.y / (self.config.num_tokens - 3) - 0.5
            loss_c = torch.nn.functional.mse_loss(decoded_c,
                                                  y_coords,
                                                  reduction='mean')
        else:
            decoded_x = decoded_x_conv.reshape(
            ...
            loss = torch.nn.functional.mse_loss(decoded_x,
                                                data.y,
                                                reduction='mean')
            loss_c = torch.nn.functional.mse_loss(decoded_x,
                                                  data.y,
                                                  reduction='mean')
        acc = self.get_accuracy(decoded_x, data.y)
        acc_triangle = self.get_triangle_accuracy(decoded_x, data.y)
        if not torch.isnan(loss).any():
            self.log(
                f"val/ce_loss{self.interesting_categories[dataloader_idx][1]}",
                loss.item(),
        ...
        if not torch.isnan(loss_c).any():
            self.log(
                f"val/mse_loss{self.interesting_categories[dataloader_idx][1]}",
                loss_c.item(),
                add_dataloader_idx=False,
                on_step=False,
                on_epoch=True,
        ...
        if not torch.isnan(commit_loss).any():
            self.log(
                f"val/embed_loss{self.interesting_categories[dataloader_idx][1]}",
                commit_loss.item(),
                add_dataloader_idx=False,
                on_step=False,
                on_epoch=True,
        ...
        if not torch.isnan(acc).any():
            self.log(
                f"val/acc{self.interesting_categories[dataloader_idx][1]}",
                acc.item(),
                add_dataloader_idx=False,
                on_step=False,
                on_epoch=True,
        ...
        if not torch.isnan(acc_triangle).any():
            self.log(
                f"val/acc_tri{self.interesting_categories[dataloader_idx][1]}",
                acc_triangle.item(),
                add_dataloader_idx=False,
                on_step=False,
                on_epoch=True,
    ...
    def on_validation_epoch_end(self):
        category_names = [""] + [
            x[0].strip("_") for x in self.interesting_categories
        ]
        for didx, dataset in enumerate([self.train_dataset] +
                                       self.val_datasets):
            ...
            for k in range(self.config.num_val_samples):
                data = dataset.get(
                    k * (len(dataset) // self.config.num_val_samples) %
                    len(dataset))
                encoded_x = self.encoder(
                    data.x.to(self.device), data.edge_index.to(self.device),
                    torch.zeros([data.x.shape[0]], device=self.device).long())
                encoded_x = encoded_x.reshape(encoded_x.shape[0] * 3, 192)
                encoded_x = self.distribute_features_fn(
                    encoded_x, data.faces.to(self.device), data.num_vertices,
                    self.device)
                encoded_x = self.pre_quant(encoded_x)
                ...
                if self.config.ce_output:
                    decoded_x = decoded_x_conv.reshape(
                        -1, decoded_x_conv.shape[-2],
                        decoded_x_conv.shape[-1])[conv_mask, :, :]
                    coords = decoded_x.argmax(-1).detach().cpu().numpy() / (
                        self.config.num_tokens - 3) - 0.5
                else:
                    decoded_x = decoded_x_conv.reshape(
                        -1, decoded_x_conv.shape[-1])[conv_mask, :]
                ...
                plot_vertices_and_faces(
                    gen_vertices, gen_faces, output_dir_image /
                    f"{self.global_step:06d}_{category_names[didx]}_{k}.jpg")
                try:
                    trimesh.Trimesh(
                        vertices=gen_vertices, faces=gen_faces, process=False
                    ).export(
                        output_dir_mesh /
                        f"{self.global_step:06d}_{category_names[didx]}_{k}.obj"
                    )
        ...
        for didx, dataset in enumerate([self.train_dataset] +
            ...
            for k in range(self.config.num_val_samples):
                ...
                plot_vertices_and_faces(
                    ...
                    output_dir_image / f"GT_{category_names[didx]}_{k}.jpg")
    ...
    def val_dataloader(self):
        ...
        for val_dataset in self.val_datasets:
            dataloaders.append(
                TriangleNodesWithFacesDataloader(
                    val_dataset,
                    batch_size=self.config.batch_size,
                    shuffle=True,
                    drop_last=True,
    ...
    def get_accuracy(self, x, y):
        if self.config.ce_output:
            return (x.argmax(-1).reshape(-1)
                    == y.reshape(-1)).sum() / (x.shape[0] * x.shape[1])
        return (quantize_coordinates(
            x, self.config.num_tokens - 2).reshape(-1) == quantize_coordinates(
                y, self.config.num_tokens -
                2).reshape(-1)).sum() / (x.shape[0] * x.shape[1])

    def get_triangle_accuracy(self, x, y):
        if self.config.ce_output:
            return torch.all(x.argmax(-1).reshape(-1, 9) == y.reshape(-1, 9),
                             dim=-1).sum() / x.shape[0]
        return torch.all(
            (quantize_coordinates(x, self.config.num_tokens - 2).reshape(
                -1, 9) == quantize_coordinates(
                    y, self.config.num_tokens - 2).reshape(-1, 9)),
            dim=-1).sum() / (x.shape[0])
