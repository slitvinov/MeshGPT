...
def accuracy(y_pred, y_true, ignore_label=None, device=None):
    ...
    if ignore_label:
        ...
        ignore_mask = torch.where(  # type: ignore
            y_true == ignore_label, torch.zeros_like(y_true, device=device),
            torch.ones_like(y_true, device=device)).type(torch.float32)
    else:
        normalizer = y_true.shape[0]
        ignore_mask = torch.ones_like(y_true,
                                      device=device).type(torch.float32)
    acc = (y_pred.reshape(-1) == y_true.reshape(-1)).type(
...
def rmse(y_pred, y_true, num_tokens, ignore_labels=(0, 1, 2)):
    mask = torch.logical_and(y_true != ignore_labels[0], y_pred
                             != ignore_labels[0])
...
def get_create_shapenet_train_val(path):
    ...
    all_categories = sorted(
        list(set(list(x.split("_")[0] for x in all_shapes))))
    counts_all = Counter()
    for s in all_shapes:
    ...
    for c in all_categories:
        print(c, f"{counts_all[c] / len(all_shapes) * 100:.2f}",
              f"{counts_val[c] / len(validation) * 100:.2f}")
...
def scale_vertices(vertices,
    ...
    vertices = np.stack(
        [vertices[:, 0] * x, vertices[:, 1] * y, vertices[:, 2] * z], axis=-1)
    return vertices


def shift_vertices(vertices,
                   x_lims=(-0.1, 0.1),
                   y_lims=(-0.1, 0.1),
                   z_lims=(-0.075, 0.075)):
    # shift x, y, z
...
def normalize_vertices(vertices):
    bounds = np.array([vertices.min(axis=0),
                       vertices.max(axis=0)])  # type: ignore
...
if __name__ == "__main__":
    ...
    logits_ = torch.FloatTensor([[3, -1, 0.5, 0.1], [0, 9, 0.5, 0.1],
                                 [0, 0, 5, 0.1]])
    for i in range(5):
