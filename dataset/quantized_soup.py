...
class QuantizedSoup(Dataset):
    ...
    def __init__(self, config, split, scale_augment):
            ...
            if config.only_chairs:
                for s in ['train', 'val']:
                    data[f'vertices_{s}'] = [
                        ...
                        if data[f'name_{s}'][i].split('_')[0] == '03001627'
                    ]
                    data[f'faces_{s}'] = [
                        data[f'faces_{s}'][i]
        ...
        for i in range(len(self.cached_faces)):
            ...
            for j in range(
                    0, max(1,
                           sequence_len - self.block_size + self.padding + 1),
    ...
    def set_quantizer(self, encoder, pre_quant, vq, post_quant, decoder,
                      device):
    ...
    def get_codes(self, vertices, faces):
        ...
        if not self.vq_is_shared:
            correction = (torch.arange(0, self.vq_depth, device=self.device) *
                          self.vq_num_codes_per_level).reshape(1, -1)
            all_indices = all_indices + correction
        inner_face_id = torch.arange(0, self.vq_depth,
                                     device=self.device).reshape(1, -1).expand(
                                         all_indices.shape[0], -1)
        outer_face_id = torch.arange(0,
                                     all_indices.shape[0],
                                     device=self.device).reshape(-1, 1).expand(
                                         -1, self.vq_depth)
        ...
        all_indices = torch.cat(
            (torch.tensor([self.start_sequence_token],
                          device=self.device), all_indices,
             torch.tensor([self.end_sequence_token],
                          device=self.device))).long().cpu()

        inner_face_id = torch.cat(
            (torch.tensor([self.start_sequence_token],
                          device=self.device), inner_face_id,
             torch.tensor([self.end_sequence_token],
                          device=self.device))).long().cpu()

        outer_face_id = torch.cat(
            (torch.tensor([self.start_sequence_token],
                          device=self.device), outer_face_id,
             torch.tensor([self.end_sequence_token],
                          device=self.device))).long().cpu()
    ...
    def __getitem__(self, index: int):
        ...
        soup_sequence, face_in_idx, face_out_idx = self.get_codes(
            vertices, faces)
        ...
        x_pad = torch.tensor([
            self.pad_face_token for _ in range(0, self.block_size - len(x_in))
        ])
        y_pad = torch.tensor([
            self.pad_face_token
            for _ in range(0,
                           len(x_in) + len(x_pad) - len(y_in))
        ])
        fpi_in_pad = torch.tensor([
            self.pad_face_token
            for _ in range(0, self.block_size - len(fpi_in))
        ])
        fpo_in_pad = torch.tensor([
            self.pad_face_token
            for _ in range(0, self.block_size - len(fpo_in))
        ])

        x = torch.cat((x_in, x_pad)).long()
        y = torch.cat((y_in, y_pad)).long()
        fpi = torch.cat((fpi_in, fpi_in_pad)).long()
        fpo = torch.from_numpy(
            get_shifted_sequence(torch.cat(
                (fpo_in, fpo_in_pad)).numpy())).long()
    ...
    def get_completion_sequence(self, i, tokens, device=torch.device("cpu")):
        ...
        soup_sequence, face_in_idx, face_out_idx = self.get_codes(
            vertices, faces)
        face_out_idx = torch.from_numpy(
            get_shifted_sequence(face_out_idx.numpy()))
    ...
    def decode(self, sequence):
        mask = torch.isin(
            ...
            torch.tensor([
                ...
                self.pad_face_token
                         ...
                         device=sequence.device)).logical_not()
        ...
        encoded_x_conv, conv_mask = create_conv_batch(
            encoded_x,
            torch.zeros([sequence.shape[0]], device=self.device).long(), 1,
            self.device)
        ...
        coords = decoded_x.argmax(
            -1).detach().cpu().numpy() / self.num_tokens - 0.5
...
class QuantizedSoupCreator(torch.nn.Module):
    ...
    def __init__(self, config, vq_cfg):
        ...
        self.encoder, self.pre_quant, self.post_quant, self.vq = self.get_rvq_encoders(
            config.vq_resume)
    ...
    def get_indices(self, x, edge_index, batch, _faces, _num_vertices):
        ...
        all_indices = all_indices.squeeze(0)
        if not self.vq_is_shared:
            correction = (torch.arange(
                0, self.vq_depth * self.vq_depth_factor, device=x.device) *
                          self.vq_num_codes_per_level).reshape(1, -1)
            all_indices = all_indices + correction
        return all_indices
    ...
    def forward(self,
        ...
        all_indices = self.get_indices(x, edge_index, batch, faces,
                                       num_vertices)
        ...
        for k in range(batch_size):
            ...
            inner_face_id_k = torch.arange(
                0, self.vq_depth * self.vq_depth_factor,
                device=x.device).reshape(1, -1).expand(sequence_k.shape[0], -1)
            outer_face_id_k = torch.arange(
                0, sequence_k.shape[0], device=x.device).reshape(-1, 1).expand(
                    -1, self.vq_depth * self.vq_depth_factor)
            ...
            inner_face_id_k = torch.cat(prefix + [inner_face_id_k] +
                                        postfix).long()

            outer_face_id_k = torch.cat(prefix + [outer_face_id_k] +
                                        postfix).long()
            ...
            if self.rq_transformer_input:
                pad_len_x = pad_len_x + (
                    self.vq_depth - (len(x_in) + pad_len_x) % self.vq_depth)
            x_pad = torch.tensor(
                [self.pad_face_token for _ in range(0, pad_len_x)],
                device=x.device)
            ...
            if self.rq_transformer_input:
                pad_len_fpi = pad_len_fpi + (
                    self.vq_depth -
                    (len(fpi_in) + pad_len_fpi) % self.vq_depth)
                pad_len_fpo = pad_len_fpo + (
                    self.vq_depth -
                    (len(fpo_in) + pad_len_fpo) % self.vq_depth)

            y_pad = torch.tensor(
                [self.pad_face_token for _ in range(0, pad_len_y)],
                device=x.device)
            fpi_in_pad = torch.tensor(
                [self.pad_face_token for _ in range(0, pad_len_fpi)],
                device=x.device)
            fpo_in_pad = torch.tensor(
                [self.pad_face_token for _ in range(0, pad_len_fpo)])
            ...
            fpo = torch.from_numpy(
                get_shifted_sequence(torch.cat(
                    (fpo_in, fpo_in_pad)).numpy())).long().to(x.device)
        ...
        sequences = torch.stack(sequences,
                                dim=0)[:, :max_sequence_length_x].contiguous()
        targets = torch.stack(targets,
                              dim=0)[:, :max_sequence_length_x].contiguous()
        position_inners = torch.stack(
            position_inners, dim=0)[:, :max_sequence_length_x].contiguous()
        position_outers = torch.stack(
            position_outers, dim=0)[:, :max_sequence_length_x].contiguous()
        return sequences, targets, position_inners, position_outers

    @torch.no_grad()
    def get_completion_sequence(self, x, edge_index, face, num_vertices,
                                tokens):
        soup_sequence, target, face_in_idx, face_out_idx = self.forward(
            x,
            edge_index,
            torch.zeros([x.shape[0]], device=x.device).long(),
            face,
            num_vertices,
    ...
    def decode(self, sequence, decoder):
        ...
        sequence_len = (sequence.shape[0] //
                        (self.vq_depth * self.vq_depth_factor)) * (
                            self.vq_depth * self.vq_depth_factor)
        sequence = sequence[:sequence_len].reshape(-1, self.vq_depth)
        ...
        encoded_x_conv, conv_mask = create_conv_batch(
            encoded_x,
            torch.zeros([encoded_x.shape[0]], device=sequence.device).long(),
            1, sequence.device)
...
class QuantizedSoupTripletsCreator(QuantizedSoupCreator):
    ...
    def get_indices(self, x, edge_index, batch, faces, num_vertices):
        ...
        encoded_x = distribute_features(encoded_x, faces, num_vertices,
                                        x.device)
        encoded_x = self.pre_quant(encoded_x)
        _, all_indices, _ = self.vq(encoded_x.unsqueeze(0))
        all_indices = all_indices.squeeze(0).reshape(-1, self.vq_depth * 3)
        if not self.vq_is_shared:
            correction = (torch.arange(0, self.vq_depth, device=x.device) *
                          self.vq_num_codes_per_level).reshape(1, -1)
            all_indices = all_indices + correction
        return all_indices
